...
/**
 * Backup-specific λειτουργίες για διαχείριση φακέλων και αρχείων ανά job.
 */
Storage.job = {
    /** Επιστρέφει το απόλυτο path του φακέλου ενός job */
    folderPath(jobName) {
        return path.resolve(Storage.storagePath, jobName);
    },

    /** Βεβαιώνεται ότι ο φάκελος του job υπάρχει, δημιουργώντας τον αν χρειαστεί */
    async ensureFolder(jobName) {
        const jobPath = this.folderPath(jobName);
        assertInStorage(jobPath);
        await fs.mkdir(jobPath, { recursive: true });
    },

    /** Επιστρέφει λίστα αρχείων backup για ένα job (ταξινομημένη νεότερα πρώτα) */
    async list(jobName) {
        const result = await Storage.list(jobName, false);
        return result.files.sort((a, b) => b.name.localeCompare(a.name));
    },

    /** Επιστρέφει το απόλυτο path ενός backup αρχείου για download (με έλεγχο ασφαλείας) */
    async downloadPath(jobName, fileName) {
        const relativePath = path.join(jobName, fileName);
        return Storage.path(relativePath);
    },

    /** Διαγράφει ένα backup αρχείο */
    async delete(jobName, fileName) {
        const relativePath = path.join(jobName, fileName);
        await Storage.delete(relativePath);
    },

    /**
     * Αποθηκεύει string περιεχόμενο ως backup αρχείο.
     * @param {string} jobName - Το name του job (χρησιμοποιείται ως φάκελος)
     * @param {string} fileName - Το όνομα αρχείου (π.χ. 2024-01-15_10-30-45.sql)
     * @param {string} content - Το περιεχόμενο του backup
     * @returns {string} Το απόλυτο path του αποθηκευμένου αρχείου
     */
    async save(jobName, fileName, content) {
        const jobFolderPath = this.folderPath(jobName);
        assertInStorage(jobFolderPath);
        await fs.mkdir(jobFolderPath, { recursive: true });

        const targetPath = path.join(jobFolderPath, fileName);
        assertInStorage(targetPath);
